...
- [4. 常用类型、连线](#4-常用类型连线)
    ...
    - [4.3. 在画图之前，先理解一下要画什么图形？要怎么连线？](#43-在画图之前先理解一下要画什么图形要怎么连线)
        - [4.3.1. 绘制抽象类Person，含基本属性和通用方法](#431-绘制抽象类person含基本属性和通用方法)
...
解释：就是一个对象facade中持有对象service的引用,这样对象service中的属性和方法facade也就知道了,就是这个意思,在实际的代码中体现在一个类上声明一个成员变量
...
解释：是整体与部分的关系, 且部分可以离开整体而单独存在,如车和轮胎是整体和部分的关系,轮胎离开车仍然可以存在  
